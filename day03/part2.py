...
inputRaw = """........................................................862...........20.............453...619......58........694...312.................292.
...
def addToGearMap(startY, startX, xLength, partNum):
    # check one row above, and one row below startY
    for y in range(startY - 1, startY + 2):
        if (y >= 0 and y < len(inputLines)):
            # check one col behind startX, and one col ahead of startX + xLength
            for x in range(startX - 1, startX + xLength + 1):
                if (x >= 0 and x < len(inputLines[0])):
                    # if it is a 'gear' symbol, add number to gearMap
                    if inputLines[y][x] == '*':
                        if (x, y) in gearMap:
                            gearMap[(x, y)].append(partNum)
                        else:
                            gearMap[(x, y)] = [partNum]



while curY < len(inputLines):
    while curX < len(inputLines[0]):
        # if it's a digit, first build the full number,
        # then figure out what gears are touching it
        if (str.isdigit(inputLines[curY][curX])):
            numStr = inputLines[curY][curX]
            numIncrement = 1
            while (curX + numIncrement) < len(inputLines[0]) and str.isdigit(inputLines[curY][curX+numIncrement]):
                numStr += inputLines[curY][curX+numIncrement]
                numIncrement += 1
            partNum = int(numStr)

            # build the gear map for that number
            addToGearMap(curY, curX, numIncrement, partNum)

            # finally, increment curX past end of the number
            curX += numIncrement
        # if not a digit, increment curX
        else:
            curX += 1
    curX = 0
    curY += 1
...
gearRatioSum = 0
for gearArr in gearMap.values():
    if len(gearArr) == 2:
        gearRatioSum += (gearArr[0] * gearArr[1])
print(gearRatioSum)
