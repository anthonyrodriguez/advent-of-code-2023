...
def adjacentSymbol(startY, startX, xLength):
        ...
        if (y >= 0 and y < len(inputLines)):
            ...
            for x in range(startX - 1, startX + xLength + 1):
                if (x >= 0 and x < len(inputLines[0])):
                    # if it's not a digit or a '.', it's a symbol
                    if not (str.isdigit(inputLines[y][x]) or inputLines[y][x] == '.'):
                        return True
    return False
...
while curY < len(inputLines):
    while curX < len(inputLines[0]):
        ...
        # then figure out if it's adjacent to a symbol
        if (str.isdigit(inputLines[curY][curX])):
            ...
            # figure out if valid partNum (adjacent to a symbol)
            if (adjacentSymbol(curY, curX, numIncrement)):
                numSum += partNum
