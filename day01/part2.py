...
inputRaw = """tsgbzmgbonethreedrqzbhxjkvcnm3
...
#inputList = testInputRaw.splitlines()
inputList = inputRaw.splitlines()
...
for inputLine in inputList:
    ...
    for i in range(0, len(inputLine)):
        if firstDigit > -1:
            break
        if str.isdigit(inputLine[i]):
            firstDigit = int(inputLine[i])
            break
        for j in range(3,6):
            if inputLine[i : i+j] in digitStrings:
                firstDigit = digitStrings.index(inputLine[i : i+j]) + 1
                break
    for i in range(len(inputLine)-1, -1, -1):
        if lastDigit > -1:
            break
        if str.isdigit(inputLine[i]):
            lastDigit = int(inputLine[i])
            break
        for j in range(2,5):
            # digitString will be backwards
            if inputLine[i-j : i+1] in digitStrings:
                lastDigit = digitStrings.index(inputLine[i-j : i+1]) + 1
                break
