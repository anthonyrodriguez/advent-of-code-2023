...
inputRaw = """L.F--FF|.-.FF-777.F7FL-F.7.F-77.F--FF--.JJ7..F-77|FFF|7-F-J7--F7|7-F7FJ7-F7FF--7--7F-|7F--.|FF|7FF-F77-F7FJ777.FJ.FJ7-|-7-FFFJFF-77L7LF|FL77
...
testInputRaw = """FF7FSF7F7F7F7F7F---7
L|LJ||||||||||||F--J
FL-7LJLJ||||||LJL-77
F--JF--7||LJLJ7F7FJ-
L---JF-JLJ.||-FJLJJ7
...
#inputGrid = [[x for x in y] for y in testInputRaw2.splitlines()]
...
# Find direction of the first pipe
nextCoords = sCoords
for y in range(-1, 2):
    for x in range(-1, 2):
        curChar = inputGrid[sCoords[1]+y][sCoords[0]+x]
        if (curChar == '|' and x == 0) or \
            (curChar == '-' and y == 0) or \
            (curChar == 'L' and ((x,y) == (0,1) or (x,y) == (-1,0))) or \
            (curChar == 'J' and ((x,y) == (0,1) or (x,y) == (1,0))) or \
            (curChar == '7' and ((x,y) == (0,-1) or (x,y) == (1,0))) or \
            (curChar == 'F' and ((x,y) == (0,-1) or (x,y) == (-1,0))):
                nextCoords = (sCoords[0]+x,sCoords[1]+y)
                break
    if nextCoords != sCoords:
        break

travelDirection = 'X'
if nextCoords[0] - sCoords[0] == 0:
    if nextCoords[1] - sCoords[1] == 1:
        travelDirection = 'S'
    else:
        travelDirection = 'N'
elif nextCoords[0] - sCoords[0] == 1:
    travelDirection = 'E'
else:
    travelDirection = 'W'

newGrid = [['.']*len(inputGrid[0]) for x in range(len(inputGrid))]

loopDistance = 0
while inputGrid[nextCoords[1]][nextCoords[0]] != 'S':
    curChar = inputGrid[nextCoords[1]][nextCoords[0]]

    newGrid[nextCoords[1]][nextCoords[0]] = curChar

    # # for better visualization if printing inputGrid
    # inputGrid[nextCoords[1]][nextCoords[0]] = '█'

    if (curChar == '|'):
        if travelDirection == 'N':
            nextCoords = (nextCoords[0], nextCoords[1]-1)
        elif travelDirection == 'S':
            nextCoords = (nextCoords[0], nextCoords[1]+1)
        else:
            print('ERROR:', travelDirection, curChar, nextCoords)
    elif (curChar == '-'):
        if travelDirection == 'E':
            nextCoords = (nextCoords[0]+1, nextCoords[1])
        elif travelDirection == 'W':
            nextCoords = (nextCoords[0]-1, nextCoords[1])
        else:
            print('ERROR:', travelDirection, curChar, nextCoords)
    elif (curChar == 'L'):
...
# # for starting 'S'
# inputGrid[nextCoords[1]][nextCoords[0]] = '█'
newGrid[nextCoords[1]][nextCoords[0]] = 'S'

enclosedTiles = 0
# for a tile to be enclosed by pipes,
# traveling left will cross an odd number of pipes ('|', 'J', 'L')
